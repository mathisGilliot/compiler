...
@init {
    ...
    : if1=IF OPARENT condition=expr CPARENT OBRACE li_if=list_inst CBRACE {
    	...
    	$tree = new IfThenElse($condition.tree, $li_if.tree, inst1);
      ...
      (ELSE elsif=IF OPARENT elsif_cond=expr CPARENT OBRACE elsif_li=list_inst CBRACE {
    	...
    	sinon = new IfThenElse($elsif_cond.tree, $elsif_li.tree, new ListInst());
    	setLocation($tree, $elsif);
    	tmp.getElseBranch().add(sinon);
        tmp=sinon;
      ...
      (ELSE OBRACE li_else=list_inst CBRACE {
      		...
      		sinon.setElseBranch($li_else.tree);
...
expr returns[AbstractExpr tree]
    : assign_expr {
            ...
            $tree = $assign_expr.tree; //a revoir pour ordre param
...
@init   {
    ...
    | m=ident OPARENT args=list_expr CPARENT {
            assert($args.tree != null);
            assert($m.tree != null);
            $tree=new MethodCall(a,$m.tree,$args.tree);
    ...
    | READINT OPARENT CPARENT { //modifié
    		$tree = new ReadInt();
...
ident returns[AbstractIdentifier tree]
    : IDENT {
    		 $tree = new Identifier(tab.create($IDENT.text));
...
class_extension returns[AbstractIdentifier tree]
    ...
    | /* epsilon */ {
    		$tree = new Identifier(tab.create("Object"));
...
@init   {
    ...
    : i=ident {
      ...
      (EQUALS e=expr {
      		...
      		a = new Initialization($e.tree);
      		setLocation(a, $EQUALS);
    		$tree = new DeclField($v, $t, $i.tree, a);
    		setLocation($tree, $i.start);
        }
      )? {
        }
...
@init {
    ...
    : type ident OPARENT params=list_params CPARENT (block {
    		...
    		assert($block.decls != null);
    		assert($block.insts != null);
		a = new MethodBody($block.decls, $block.insts);
		setLocation(a, $block.start);
    	   	$tree = new DeclMethod($type.tree, $ident.tree, $params.tree, a);
        }
      | ASM OPARENT code=multi_line_string CPARENT SEMI {
		...
		a = new MethodBodyAsm(new StringLiteral($code.text));
		setLocation(a, $ASM);
      		$tree = new DeclMethod($type.tree, $ident.tree, $params.tree, a);
